    ...
    public static function the_content($content) {
        ...
        for ($i = 1; $i <= count($pages); $i++) {
            ...
            if ($i != $currentPage) {
                ...
                // Apply filters.
                $slideContent = self::$beginComment . $slideContent . self::$endComment;
                 ...
                 */
                $begin = TpsMisc::mb_strpos($slideContent, self::$beginComment);
                $end = TpsMisc::mb_strpos($slideContent, self::$endComment);
                if ($begin !== false && $end !== false) {

                    // Preserve beginning <p> tag.
                    if (TpsMisc::mb_substr($slideContent, $begin - 3, 3) == '<p>') {
                        if (TpsMisc::mb_substr($slideContent, $begin + TpsMisc::mb_strlen(self::$beginComment), 4) == '</p>') {
                            $begin += TpsMisc::mb_strlen(self::$beginComment) + 4;
                        }
                        else {
                            $begin -= 3;
                        }
                        ...
                        $begin += TpsMisc::mb_strlen(self::$beginComment);
                    }

                    // Preserve ending <p> tag.
                    if (TpsMisc::mb_substr($slideContent, $end + TpsMisc::mb_strlen(self::$endComment), 4) == '</p>') {
                        if (TpsMisc::mb_substr($slideContent, $end - 3, 3) == '<p>') {
                            $end -= 3;
                        }
                        else {
                            $end += TpsMisc::mb_strlen(self::$endComment) + 4;
                        }
                    }
